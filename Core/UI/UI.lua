...
--]]	
	...
	if private.LAST_NOTEWOTHYITEM_UI2 then
        private.LAST_NOTEWOTHYITEM_UI2:Show()
        return
    end
...
--------------------------------------------------------------------------
	...
	private.LAST_NOTEWOTHYITEM_UI2 = CreateFrame("Frame",nil,UIParent, "BasicFrameTemplate" or nil)
	private.LAST_NOTEWOTHYITEM_UI2:Hide()
	private.LAST_NOTEWOTHYITEM_UI2:SetSize(350,30)
	--158 -515
	private.LAST_NOTEWOTHYITEM_UI2:SetPoint("top", UIParent, "left", 250, 5);
    private.LAST_NOTEWOTHYITEM_UI2:CreateFontString(nil, "OVERLAY");
	
	private.LAST_NOTEWOTHYITEM_UI2.title = private.LAST_NOTEWOTHYITEM_UI2:CreateFontString(nil, "OVERLAY");
	private.LAST_NOTEWOTHYITEM_UI2.title:SetFontObject("GameFontHighlight");
	private.LAST_NOTEWOTHYITEM_UI2.title:SetPoint("CENTER", private.LAST_NOTEWOTHYITEM_UI2.TitleBg, "CENTER", 0, 0);
	private.LAST_NOTEWOTHYITEM_UI2.title:SetText("Last Noteworthy Item");

	--make dragable by mouse
	private.LAST_NOTEWOTHYITEM_UI2:SetMovable(true)
	private.LAST_NOTEWOTHYITEM_UI2:EnableMouse(true)
	private.LAST_NOTEWOTHYITEM_UI2:RegisterForDrag("LeftButton")
	private.LAST_NOTEWOTHYITEM_UI2:SetScript("OnDragStart", private.LAST_NOTEWOTHYITEM_UI2.StartMoving, pickupDragMovement())
	private.LAST_NOTEWOTHYITEM_UI2:SetScript("OnDragStop", private.LAST_NOTEWOTHYITEM_UI2.StopMovingOrSizing)

	private.LAST_NOTEWOTHYITEM_UI2:Show()

	--map this back to variables file	
--------------------------------------------------------------------------
--------------------------------------------------------------------------
...
--]]	
    ...
    if private.liteUI2 then
        private.liteUI2:Show()
        return
    end
...
--------------------------------------------------------------------------
-- New Lite UI frame 11.22.2020
--------------------------------------------------------------------------
	private.liteUI2 = CreateFrame("Frame",nil,UIParent, "BasicFrameTemplate" or nil)
	private.liteUI2:SetSize(150,30)

	
	--***** READ / WRITE COORDS OF POSITIONS LAST MOVED TO THEN UPDATE SETPOINTS
	local topCoord = LA.GetFromDb("liteUI", "top")
	local leftCoord = LA.GetFromDb("liteUI", "left")
	LA.Debug.Log("top:" .. topCoord)
	LA.Debug.Log("left:" .. leftCoord)
	
	private.liteUI2:SetPoint("top", UIParent, "left", 277, -72);
    private.liteUI2:CreateFontString(nil, "OVERLAY");

	private.liteUI2.title = private.liteUI2:CreateFontString(nil, "OVERLAY");
	private.liteUI2.title:SetFontObject("GameFontHighlight");
	private.liteUI2.title:SetPoint("CENTER", private.liteUI2.TitleBg, "CENTER", 0, 0);
	private.liteUI2.title:SetText("|cffffffff" .. LA.Util.MoneyToString(totalItemValue) .. "|r");
	--make dragable by mouse
	private.liteUI2:SetMovable(true)
	private.liteUI2:EnableMouse(true)
	private.liteUI2:RegisterForDrag("LeftButton")
	private.liteUI2:SetScript("OnDragStart", private.liteUI2.StartMoving, pickupDragMovement())
	private.liteUI2:SetScript("OnDragStop", private.liteUI2.StopMovingOrSizing)
	--map this back to variables file
...
function UI.RefreshStatusText()
    if private.MAIN_UI ~= nil then
        ...
        local preparedText = "Filter: " .. LA.CONST.QUALITY_FILTER[tostring(LA.GetFromDb("notification", "qualityFilter"))] 						-- filter
        preparedText = preparedText .. " - GAT-A: |cffffffff" .. LA.GetFromDb("notification", "goldAlertThresholdA") .. "|cffffd100g|r"				-- gatA
        --preparedText = preparedText .. " - GAT-B: |cffffffff" .. LA.GetFromDb("notification", "goldAlertThresholdB") .. "|cffffd100g|r"				-- gatB
        --preparedText = preparedText .. " - GAT-C: |cffffffff" .. LA.GetFromDb("notification", "goldAlertThresholdC") .. "|cffffd100g|r"				-- gatC
...
--]]	
	if private.LAST_NOTEWOTHYITEM_UI2 then
        private.LAST_NOTEWOTHYITEM_UI2.title:SetText(itemLink .. "|cffffffff x" .. quantity .. ": " .. formattedValue .. "|r")
    end
...
function UI.UpdateLiteWindowUI(formattedValue)
	...
	if private.liteUI2 then
	    local totalItemValue = LA.Session.GetCurrentSession("liv") or 0
		private.liteUI2.title:SetText("|cffffffff" .. LA.Util.MoneyToString(totalItemValue) .. "|r");
		LA.Debug.Log("Updating LA Lite 2 value: " .. tostring(totalItemValue))
    end	
end
...
--]]	
	if private.LAST_NOTEWOTHYITEM_UI2 then
        private.LAST_NOTEWOTHYITEM_UI2.title:SetText("Gold Alert Threshold")
    end
...
--]]	
	if private.LAST_NOTEWOTHYITEM_UI2 then
        private.LAST_NOTEWOTHYITEM_UI2:ApplyStatus()
    end
	...
	LA.db.profile.liteUI2 = { ["height"] = 30, ["top"] = (parentHeight-135), ["left"] = 50, ["width"] = 150, }
    if private.liteUI2 then
        private.liteUI2:ApplyStatus()
    end
